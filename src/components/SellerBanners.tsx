...
export function SellerBanners() {
  ...
  const Inner = (
    <div className="relative aspect-[16/6] sm:aspect-[16/5] bg-secondary">
      <img src={b.image_url!} alt={b.title} className="w-full h-full object-cover" />
      <div className="absolute top-2 left-2 bg-warning text-warning-foreground text-[10px] font-bold px-2 py-0.5 rounded-full inline-flex items-center gap-1">
        <Sparkles className="h-3 w-3" /> REKLAM
      </div>
      {banners.length > 1 && (
